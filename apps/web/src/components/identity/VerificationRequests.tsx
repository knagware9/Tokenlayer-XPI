  ...
  return (
    <div className="space-y-5">
      <Card title="Request a presentation" description="Ask a holder to present specific credentials. They consent and choose what to disclose.">
        ...
        <label className="block text-xs text-muted mb-1">Credential use case (optional)</label>
        ...
        {typeNames.length > 1 && <label className="block text-xs text-muted mb-1">Credential schema</label>}
            ...
            return (
              <div key={t}>
                {typeNames.length > 1 ? (
                  ...
                  <div className="text-sm text-muted">Credential schema: <span className="font-medium text-fg">{t}</span></div>
                )}
                {picked[t] && Object.keys(props).length > 0 && (
                  <div className="ml-5 mt-1 space-y-1 border-l border-border pl-3">
                    <div className="text-[11px] text-muted">Request specific fields (optional)</div>
                            ...
                            <>
                              ...
                              {fr.kind === "predicate" && (
                                <>
                                  ...
                                  <input
                                    type="number" className="w-20 rounded-lg border border-border bg-elevated/80 px-2.5 py-1 text-xs focus:outline-none focus:border-primary focus:bg-surface focus:ring-4 focus:ring-primary/15"
                                    value={fr.threshold}
                                    onChange={(e) => {
        ...
        <div className="flex gap-2">
          ...
          {reqId && <button className="rounded-lg border border-border px-4 py-2 text-sm" onClick={() => void runVerify()}>Run verification</button>}
        </div>
      </Card>
            ...
            {outbox.map((r) => (
              <li key={r.id} className={`py-3 flex items-start justify-between gap-4 ${r.id === reqId ? "bg-brand-50/40 -mx-2 px-2 rounded-lg" : ""}`}>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {r.status === "consented" && (
                    <button className="rounded-lg border border-border px-3 py-1.5 text-xs" onClick={() => void runVerify(r.id)}>Run verification</button>
                  )}
                </div>
              </li>
            ))}
      ...
      {result && (
        <Card title="Verification result" description={result.valid ? "Presentation is valid." : "Presentation did not fully verify."}>
          <div className="mb-2"><Pill tone={result.valid ? "ok" : "danger"}>{result.valid ? "valid" : "invalid"}</Pill> <span className="text-xs text-muted">holder {result.holderDid?.slice(0, 20)}…</span></div>
            ...
            {result.credentials.map((c, i) => {
              ...
              return (
                <div key={i} className="border border-border rounded-lg p-3">
                  <div className="font-medium">{c.type ?? "unknown credential"} {c.reason && !reasonOnFailingRow && <span className="text-xs text-danger">· {c.reason}</span>}</div>
                  {checks ? (
                      ...
                      {CHECK_ROWS.map(({ key, label }) => {
                        ...
                        return (
                          <div key={key} className="text-xs">
                            ...
                            {key === firstFail && c.reason && <div className="ml-7 mt-0.5 text-[11px] text-danger">{c.reason}</div>}
                            {key === "trusted" && c.issuerResolution && <div className="ml-7 mt-0.5">{issuerPill(c.issuerResolution)}</div>}
                            {key === "notRevoked" && c.issuerResolution && <div className="ml-7 mt-0.5 text-[11px] text-muted">checked on-chain</div>}
                          </div>
                        );
                      })}
                    </div>
