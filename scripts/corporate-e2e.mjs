...
console.log("\n== 2) The platform admin sees it in the approval queue ==");
...
console.log("\n== 3) Approve → org DID registers on-chain + admin gets a membership VC + can log in ==");
...
console.log(`\n${fails ? `❌ ${fails} CHECK(S) FAILED` : "✅ CORPORATE SELF-SERVICE END-TO-END PASSED — public self-registration → platform approval (org DID registered on real Besu, proven by eth_call) → membership VC + login → gated use-case config (SoD enforced) → org-owned deploy → the corporate onboards an Issuer and mints an asset"}`);
