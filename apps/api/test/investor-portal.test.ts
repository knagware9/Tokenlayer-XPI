...
describe("investor portal endpoints", () => {
  ...
  it("activity: redemption share matches what settlement actually paid", async () => {
    ...
    // treasury when `from` is omitted below — fund THAT account, not an
    // arbitrary wallet, so the settlement has the cash to pay out.
    const treasury = await treasuryAddressOf(app, platform, "invoice-tokenization");
    await app.inject({ method: "POST", url: `${V1}/cash/credit`, headers: auth(platform), payload: { account: treasury, currency: "CBDC-INR", amount: "1000000" } });
