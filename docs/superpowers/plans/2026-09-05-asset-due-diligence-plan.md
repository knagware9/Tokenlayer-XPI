...
- [ ] **Step 5 finding — a real, live-only security bug this task's job is exactly to catch**

Step 5.3's direct `GET /documents/:id` attempt did NOT 403 on the first live pass — it returned 200 with the real prospectus bytes, for every caller including a PlatformAdmin. Every automated test (including Task 2's own `asset-diligence-documents.test.ts` test 5, written for exactly this refusal) passed throughout Tasks 1-8, because the fast suite only ever exercises `MemoryDocumentRepository`.

Root cause, found by reading `PrismaDocumentRepository.get()` (`apps/api/src/persistence/prisma/shared.ts`) directly: it re-enumerated valid purposes by hand — `r.purpose === "brand-logo" || r.purpose === "kyc" ? r.purpose : null` — silently returning `null` for any value outside that list. `"asset-diligence"` (added by Task 2) was never added to this hand-written list, so the repository handed back `purpose: null` regardless of what the database actually stored (confirmed via a direct Prisma query: the column genuinely held `"asset-diligence"`), and `GET /documents/:id`'s `if (doc.purpose === "asset-diligence")` refusal could never fire. This is the identical bug class already found once during the KYC feature's final review, for a different purpose value — the fix here is the same: cast through `DocumentPurpose` instead of re-enumerating literals, so a future added purpose value can never silently regress this same way again.

Fixed in `apps/api/src/persistence/prisma/shared.ts` (commit `d6a3974`), verified via a one-off script against a real SQLite file (create with `purpose: "asset-diligence"`, read back — survived), then verified live: rebuilt and redeployed just `tokenization-api`, re-ran the exact failing `GET /documents/:id` call (now 403), and re-confirmed the legitimate `GET /assets/:id/diligence/documents/:docId` path still returns the real bytes (200) and the other two Step 5 security checks (cross-use-case decide refusal, cross-use-case buyer 404) still hold post-redeploy.

**This is the reason Task 9 exists as a real, non-optional step in this plan and not a formality**: a task-scoped code review, however careful, cannot see a defect that only manifests when the real repository implementation is exercised — every one of Tasks 1-8's reviews (including two rounds of dedicated security-property tracing in Tasks 2 and 4) read this exact refusal's *route* code and judged it correct, because it was — the bug was one layer below, in code none of those tasks' diffs touched. The plan's own Global Constraints called this exact risk out for `PrismaAssetRepository` (hence Task 1's hand-verification script) but did not extend the same suspicion to `PrismaDocumentRepository`, which is precisely where it turned out to live.

---

## Final whole-branch review and fix wave

After all 9 tasks, a final review (dispatched on the most capable available model per this process's own Model Selection rule, since per-task reviews had already covered everything a task-scoped diff could reveal) found 1 Critical and 8 Important findings that only exist at the whole-branch, integration level:

1. **Critical**: `POST /assets/:id/submit-for-review` never actually returned a rejected asset to `pending_approval` — the entire resubmission flow the spec, the UI copy, and a test's own name all promised was a permanent dead end. The Task 3 test named for exactly this case checked only `statusCode === 200`, never that the asset became reviewable again.
2. Issuance-fee double refund once #1 is fixed (`pendingIssuanceFee` was never cleared after refunding).
3. Diligence documents were swappable on an already-active asset (no status guard on the upload route).
4. Concurrent approvals could double-mint supply — the status check and the mint were separated by several `await`s with no atomicity.
5. `Asset#`'s `additionalProperties: true` on `dueDiligence` leaked internal activation plumbing (including the issuer's own wallet address) and a reviewer's private rejection reason to every buyer who could read the asset at all.
6. The Review Assets screen had no way to actually open and read a document before deciding on it.
7. Task 8's blast-radius migration covered `apps/api/test/**` but missed `apps/api/src/dev/*.ts`'s demo/e2e scripts.
8. A use case with no onboarded UseCaseAdmin had no eligible decider — **this overturns Task 8's own ruling** that called the equivalent gap "pre-existing": before this feature such a use case activated synchronously; after it, it's permanently stuck, which is a real regression this feature introduced, not a pre-existing platform gap.

All 8 were fixed in one consolidated fix wave (this process allows exactly one, not a wave per finding) — see commits `a405f60`, `8a35112`, `2a024cd`, `fc6b8fc`, `78bb083`, `efae27e`. A scoped re-review confirmed all 8 ADDRESSED, independently tracing the compare-and-set logic (#4) and the response-projection field list (#5) rather than trusting the fix report's claims. One Minor, non-blocking behavior change was found in the fix diff itself (an error-code precedence change in `review-decision` when both a body-validation failure and a non-pending status apply — 400 now wins over 409; no security implication) and was parked rather than triggering another round.

**One item was NOT fixed and is disclosed rather than silently dropped**: the final review's own triage separately flagged a lost-update race in `AssetRepository.setDueDiligence`'s read-modify-write shape (two near-simultaneous document uploads to the same asset can clobber each other's slot) as worth fixing. The fix-wave dispatch scoped item 4's compare-and-set specifically to the `review-decision` status transition (the one with real financial/double-mint consequences) and did not extend it to the general document-upload path. This is a real, moderate-severity gap (affects document references, not money or activation) left open at the end of this plan's execution — surfaced explicitly here and in the session's final summary rather than given an unauthorized second fix wave.
