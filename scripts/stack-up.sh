...
if [ ! -f "$ENV_FILE" ]; then
  say "minting $ENV_FILE (first run)"
  {
    ...
    echo "# PERSONAS_-prefixed on purpose — see docker-compose.identity.yml's"
    echo "# JWT_SECRET guard comment: the plain names collide with the repo"
    echo "# root's separate .env (single-stack docker-compose.yml)."
    echo "PERSONAS_JWT_SECRET=$(openssl rand -hex 32)"
    echo "PERSONAS_DID_MASTER_KEY=$(openssl rand -hex 32)"
  } > "$ENV_FILE"
fi
