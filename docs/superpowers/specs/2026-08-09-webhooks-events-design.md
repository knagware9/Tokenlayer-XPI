...
- **Auto-disable:** 20 consecutive failed *deliveries* (not attempts) disables the endpoint with a reason, and it must be re-enabled explicitly. Without this, one abandoned integration retries forever.
...
Started from `server.ts`, not `app.ts`, so the test harness never starts a live worker; tests drive `dispatchDue()` directly, which is the whole worker minus the timer. Stopped on `SIGTERM`/`SIGINT` so a shutdown doesn't strand `inflight` rows; a startup sweep also reclaims rows left `inflight` longer than a stale threshold (crash recovery).
