...
ok(summary.tampered.length === 0, `summary: ${summary.verified}/${summary.assets} verified, 0 tampered`);
...
const rewrite = `const {createHash}=require('crypto');const {PrismaClient}=require('@prisma/client');const p=new PrismaClient();
...
(async()=>{const rows=await p.auditLog.findMany({where:{assetId:'${assetId}'},orderBy:{seq:'asc'}});
 ...
 for(const r of rows){const actorId=r.seq===1?'attacker':r.actorId;
...
const out2 = inContainer(rewrite);
...
console.log(`\n${fails ? `❌ ${fails} CHECK(S) FAILED` : "✅ HASH-CHAINED AUDIT E2E PASSED — partial tamper caught by the chain; full consistent rewrite caught by the on-ledger anchor"}`);
