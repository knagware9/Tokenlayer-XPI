...
describe("API key auth seam (EN-B task B3)", () => {
  ...
  it("populates request.apiKey for a key request, leaves it undefined for a JWT, and hands routes a COPY of the scopes", async () => {
    ...
    // Kept as an EXACT-shape assertion rather than relaxed to toMatchObject:
    // the point of this line is that the principal carries nothing beyond
    // what a route is meant to read, and a field appearing here without
    // anyone noticing is precisely what it catches.
